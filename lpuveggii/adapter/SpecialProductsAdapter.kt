...
class SpecialProductsAdapter(var c  : Context) : Adapter<SpecialProductsAdapter.specialClassViewholder>() {
    lateinit var cartdata: cartEntityClass

    private val room by lazy {
        Room.databaseBuilder(c, cartDatabase::class.java, "cartEntityClass")
            .fallbackToDestructiveMigration().allowMainThreadQueries().build()
    }

    private val originalDataRoom by lazy {
        Room.databaseBuilder(c, originalDataclass::class.java, "originaldataEntityClas")
            .fallbackToDestructiveMigration().allowMainThreadQueries().build()
    }
    lateinit var originalDataEntityClass: originaldataEntityClas

    inner class specialClassViewholder(v: View) : ViewHolder(v) {
        val b = MostlovedproductsBinding.bind(v)
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): specialClassViewholder {
        return specialClassViewholder(
            LayoutInflater.from(c).inflate(R.layout.mostlovedproducts, parent, false)
        )
    }
...
//                holder.b.price.text = listt[position].price
//                holder.b.gram.text = listt[position].quantity.toString()
//                holder.b.sizetv.text = listt[position].unit.toString()
            }
        }
